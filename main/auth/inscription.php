...
if (api_get_configuration_value('user_registration_save_session_id')) {
    ...
    $sessionIdToSave = isset($_REQUEST['s']) && !empty($_REQUEST['s']) ? $_REQUEST['s'] : null;
}
...
if ($userGateKeeperEnabled) {
    $defaults['session_id'] = $sessionIdToSave;
}
    ...
    if ($formValid) {
        ...
        if ($user_already_registered_show_terms &&
            ...
            if ($saveSessionRegistration && !empty($sessionToSave)) {
                ...
                $sessionInfoToSave = SessionManager::fetch($sessionToSave);

                if (!empty($sessionInfoToSave)) {
                    $extras['user_registration_session_name'] = $sessionInfoToSave['name'];
                }
            }
            ...
            // Creates a new user (disable automatic email sending to control order)
            ...
            if ($userGateKeeperEnabled) {
                ...
                UserGatekeeper::addUserSession($user_id, $sessionId, true);
            }
            ...
            if ($user_id) {
                ...
                if (!empty($sessionToRedirect) && !$sessionPremiumChecker) {
                    ...
                    if (!empty($sessionInfo)) {
                        ...
                        // Send campus registration email AFTER session enrollment
                        // This ensures the correct order: session email first, then campus email
                        ...
                        if (!empty($userInfo) && !empty($userInfo['email'])) {
                            ...
                            $tplSubject->assign('complete_name', stripslashes(api_get_person_name($values['firstname'], $values['lastname'])));
                            ...
                            $tplContent->assign('complete_name', stripslashes(api_get_person_name($values['firstname'], $values['lastname'])));
                            $tplContent->assign('login_name', $values['username']);
                            $tplContent->assign('original_password', stripslashes($values['pass1']));
                            $tplContent->assign('mailWebPath', $url);
                            $tplContent->assign('new_user', $userInfo);
                    ...
                    // Send campus registration email for users not enrolling in a session
                    $userInfo = api_get_user_info($user_id);
                    if (!empty($userInfo) && !empty($userInfo['email'])) {
                        $recipient_name = api_get_person_name(
                            $values['firstname'],
