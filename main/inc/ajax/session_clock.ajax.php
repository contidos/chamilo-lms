...
if(isset($_GET['action'])) {
    ...
    if($action == 'time') {
        ...
        if($endTime > 0) {
            echo json_encode(['sessionEndDate' => $endTime, 'sessionTimeLeft' => $timeLeft, 'sessionExpired' => $isExpired, 'extraData' => $host]);
        } else {
        ...
        error_log('Closing session automatically for user '.$userId);
