...
error_log(__FILE__ . " : " . __LINE__ . " before foreach");
    foreach ($htmlList as $fileName => $content) {
        ...
        if ($exportZip) {
            ...
            $fileList[] = $filePath;
error_log(__FILE__ . " : " . __LINE__ . " # filePath = " . $filePath);
        } else {
            $pdf->content_to_pdf($content, '', $fileName, null, 'D', false, null, false, false, false);
        }
    }

error_log(__FILE__ . " : " . __LINE__ . " after foreach");
    if (!empty($fileList)) {
error_log(__FILE__ . " : " . __LINE__ . " fileList = " . print_r($fileList,true));
